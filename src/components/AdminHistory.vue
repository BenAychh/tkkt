...
const getChangeMessage = (item: CRDTMessage<Admin>, previousState: Admin): string => {
  const name = previousState?.name ? previousState.name : 'this admin'
...
const props = defineProps<{ history: CRDTMessage<Admin>[] }>()
...
const groupedByBatchId = props.history.reduce((acc, item: CRDTMessage<Admin>) => {
  if (!acc.length) {
    return [[item]]
  }
