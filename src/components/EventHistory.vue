...
const getChangeMessage = (item: CRDTMessage<Event>, previousState: Event): string => {
...
const props = defineProps<{ history: CRDTMessage<Event>[] }>()
let itemName = `this event`
...
const groupedByBatchId = props.history.reduce((acc, item: CRDTMessage<Event>) => {
  if (!acc.length) {
    return [[item]]
  }
  ...
  <div>
    <div v-for="item in historyItems" :key="item.key" class="mb-3">
      <div>
        ...
        <span v-if="item.actor" class="font-bold">&nbsp;{{ item.actor }}</span>
      </div>
      <div>
        {{ item.message }}
      </div>
